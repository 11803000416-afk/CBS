...
@section('subtitle', 'Manage buyer inquiries and communications')

@section('content')
<div class="flex justify-between items-center mb-6">
    <p class="text-sm text-slate-600">Total: <span class="font-semibold text-slate-800">{{ $inquiries->total() }}</span> inquiries</p>
    <a href="{{ route('inquiries.create') }}" class="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white px-5 py-2.5 rounded-lg shadow-md hover:shadow-lg transition-all flex items-center gap-2 font-medium">
        ...
        <div class="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden hover:shadow-lg transition-all">
            ...
            <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-5 py-3 border-b border-slate-200 flex items-center justify-between">
                ...
                <span class="inline-flex px-3 py-1 rounded-full text-xs font-semibold
                    ...
                    {{ $inquiry->status === 'closed' ? 'bg-slate-100 text-slate-700' : '' }}">
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-slate-800">{{ $inquiry->vehicle->brand ?? '-' }} {{ $inquiry->vehicle->model ?? '' }}</p>
                    </div>
                </div>

                <!-- Message -->
                <div>
                    <p class="text-sm text-slate-600 leading-relaxed">{{ $inquiry->message }}</p>
                </div>

                <!-- Meeting Details -->
                @if($inquiry->meeting_location || $inquiry->preferred_time || $inquiry->special_requirements)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3 border-t border-slate-200">
                        @if($inquiry->meeting_location)
                            <div class="flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-slate-700">{{ $inquiry->meeting_location }}</p>
                                </div>
                            </div>
                        @endif
                        @if($inquiry->preferred_time)
                            <div class="flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-slate-700">{{ $inquiry->preferred_time->format('M d, Y - h:i A') }}</p>
                                </div>
                            </div>
                        @endif
                        @if($inquiry->special_requirements)
                            <div class="md:col-span-2 flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-700">{{ $inquiry->special_requirements }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                @if($inquiry->response)
                    <div class="p-3 bg-emerald-50 rounded-lg border border-emerald-100">
                        ...
                        <p class="text-sm text-slate-700">{{ $inquiry->response }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-slate-50 px-5 py-3 border-t border-slate-200 flex items-center justify-end gap-3">
                <a href="{{ route('inquiries.edit', $inquiry) }}" class="text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1 transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                    Edit
                </a>
                <form method="POST" action="{{ route('inquiries.destroy', $inquiry) }}" onsubmit="return confirm('Are you sure you want to delete this inquiry?')" class="inline">
                    @csrf @method('DELETE')
