@props(['vehicle', 'showViewButton' => true, 'showBrowseButton' => true])
...
<div class="vehicle-card group">
    ...
    <div class="vehica-card-image-container">
        ...
        <div class="vehica-price-badge">
            {{ $vehicle->currency ?? 'Nu.' }}{{ number_format($vehicle->price ?? 0, 0) }}
        </div>

        <div class="absolute left-4 top-4 flex items-center gap-2">
            <span class="premium-badge bg-white/90 text-slate-800 shadow-md backdrop-blur">{{ $vehicle->year }}</span>
        </div>
    </div>
    ...
    <div class="vehica-card-content">
        ...
        <div class="vehica-card-header">
            <div>
                <h3 class="vehica-card-title">
                    {{ $vehicle->brand }} <span class="text-cyan-600">{{ $vehicle->model }}</span>
                </h3>
                <p class="vehica-card-year">{{ ucfirst($vehicle->status ?? 'available') }} • Listed {{ optional($vehicle->created_at)->format('M d, Y') }}</p>
            </div>
        </div>
        ...
        <div class="vehica-specs-row gap-3 border-0 pb-0">
            <div class="vehica-spec vehicle-card-meta-item">
                ...
                <p class="vehica-spec-value">{{ number_format($vehicle->mileage ?? 0) }} km</p>
            </div>
            <div class="vehica-spec vehicle-card-meta-item">
                <p class="vehica-spec-label">Condition</p>
                <p class="vehica-spec-value">{{ $vehicle->condition ?? 'Verified' }}</p>
            </div>
        </div>

        <!-- Action Buttons -->
        @if($showViewButton || $showBrowseButton)
            <div class="vehica-button-group">
                @if($showViewButton)
                    <a href="{{ route('vehicles.show', $vehicle) }}" class="vehica-btn vehica-btn-primary">
                ...
                @endif
                @if($showBrowseButton)
                    <a href="{{ route('vehicles.unified') }}" class="vehica-btn vehica-btn-secondary">
