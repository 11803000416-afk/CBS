    ...
    <div class="dashboard-role-shell py-8">
        ...
        <div class="mb-8 grid grid-cols-1 gap-5 md:grid-cols-3 lg:gap-6">
            <div class="bg-white rounded-2xl shadow-md p-6 border-l-4 border-red-500">
                ...
                <p class="text-red-600 text-xs mt-2">{{ round(($stats['sold_vehicles'] / max($stats['vehicles'], 1)) * 100, 1) }}% of total</p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-md p-6 border-l-4 border-blue-500">
                ...
                <p class="text-3xl font-bold text-gray-900">{{ $stats['completed_transactions'] }}</p>
                <p class="text-blue-600 text-xs mt-2">{{ round(($stats['completed_transactions'] / max($stats['total_transactions'], 1)) * 100, 1) }}% complete rate</p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-md p-6 border-l-4 border-amber-500">
                ...
                <p class="text-3xl font-bold text-gray-900">{{ $stats['pending_inquiries'] }}</p>
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('vehicles.unified') }}" class="text-blue-600 hover:text-blue-700 font-semibold text-sm">View All →</a>
            </div>
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                            <div class="flex items-center flex-1">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-600">{{ $seller->email }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="font-bold text-blue-600">{{ $seller->vehicles_count }}</p>
...
<script>
    ...
    new Chart(revenueCtx, {
        type: 'bar',
        data: {
            labels: {!! json_encode($transactionsByMonth['labels']) !!},
            datasets: [{
                label: 'Revenue (×100K)',
                data: {!! json_encode($transactionsByMonth['data']) !!},
    ...
    new Chart(statusCtx, {
        type: 'doughnut',
        data: {
            labels: {!! json_encode($vehicleStatusBreakdown['labels']) !!},
            datasets: [{
                data: {!! json_encode($vehicleStatusBreakdown['data']) !!},
                backgroundColor: [
                    '#10b981',
                    '#8b5cf6',
                    '#f59e0b',
