...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('transactions.create') }}" class="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white px-5 py-2.5 rounded-lg shadow-md hover:shadow-lg transition-all flex items-center gap-2 font-medium">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($transactions as $transaction)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 py-4 font-semibold text-emerald-600">Nu. {{ number_format($transaction->sale_price, 2) }}</td>
                        <td class="px-4 py-4 font-semibold text-blue-600">Nu. {{ number_format($transaction->broker_commission, 2) }}</td>
                        <td class="px-4 py-4">
                            <span class="inline-flex px-3 py-1 rounded-full text-xs font-semibold {{ $transaction->status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">
                                {{ ucfirst($transaction->status) }}
                            </span>
                        </td>
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-3">
                                <a href="{{ route('transactions.edit', $transaction) }}" class="text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1 transition">
