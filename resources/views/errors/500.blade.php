        ...
        <div class="dashboard-card text-center">
            ...
            @if(isset($reference))
                <div class="mt-6 rounded-lg bg-red-50 border border-red-200 p-4">
                    <p class="text-sm text-gray-700">Reference code: <strong class="text-red-700 font-mono text-lg">{{ $reference }}</strong></p>
                    <p class="text-xs text-gray-500 mt-1">Share this code with the support team for faster troubleshooting.</p>
                </div>
            @endif
            <div class="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
                <a href="{{ route('dashboard') }}" class="btn-primary inline-flex justify-center">Return to dashboard</a>
                <a href="{{ url()->previous() }}" class="btn-secondary inline-flex justify-center">Try again</a>
            </div>
        </div>
    </div>
</body>
