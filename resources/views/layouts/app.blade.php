...
<head>
    ...
    <title>@yield('title', 'Carle') - Car Trading Platform</title>
...
<body class="aurora-bg min-h-screen text-white">
    <div class="min-h-screen">
            ...
            <div class="max-w-7xl mx-auto px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-8">
                        <a href="{{ route('dashboard') }}" class="text-2xl font-bold text-white hover:text-blue-400 transition">
                            Carle
                        </a>
                        <!-- Main Navigation Links -->
                        <div class="hidden lg:flex items-center gap-6">
                            <a href="{{ route('dashboard') }}" class="text-sm font-medium {{ request()->routeIs('dashboard') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                Home
                            </a>
                            @if(auth()->user()->hasRole(['admin','agent']))
                                <a href="{{ route('vehicles.index') }}" class="text-sm font-medium {{ request()->routeIs('vehicles.*') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                    Vehicles
                                </a>
                                <a href="{{ route('buyers.index') }}" class="text-sm font-medium {{ request()->routeIs('buyers.*') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                    Buyers
                                </a>
                                <a href="{{ route('sellers.index') }}" class="text-sm font-medium {{ request()->routeIs('sellers.*') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                    Sellers
                                </a>
                                <a href="{{ route('transactions.index') }}" class="text-sm font-medium {{ request()->routeIs('transactions.*') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                    Transactions
                                </a>
                                <a href="{{ route('reports.index') }}" class="text-sm font-medium {{ request()->routeIs('reports.*') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                    Reports
                                </a>
                            @else
                                <a href="{{ route('my-vehicles.index') }}" class="text-sm font-medium {{ request()->routeIs('my-vehicles.*') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                    My Listings
                                </a>
                            @endif
                            <a href="{{ route('inquiries.index') }}" class="text-sm font-medium {{ request()->routeIs('inquiries.*') ? 'text-blue-400' : 'text-gray-300 hover:text-white' }}">
                                Inquiries
                            </a>
                        </div>
                    </div>
        ...
        <main class="max-w-7xl mx-auto px-6 py-8">
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-white mb-2">@yield('title', 'Dashboard')</h1>
                <p class="text-gray-400">@yield('subtitle', 'Manage your car trading operations')</p>
            </div>
