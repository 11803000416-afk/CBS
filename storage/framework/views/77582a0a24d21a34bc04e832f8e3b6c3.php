...
<?php $__env->startSection('subtitle', 'Manage vehicle inventory and listings'); ?>
...
<div class="glass-card rounded-xl p-6 mb-6">
    ...
    <form method="GET" action="<?php echo e(route('vehicles.index')); ?>" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-7 gap-4">
        <input name="brand" value="<?php echo e(request('brand')); ?>" placeholder="Brand" class="bg-white/10 border border-white/20 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
        <input name="model" value="<?php echo e(request('model')); ?>" placeholder="Model" class="bg-white/10 border border-white/20 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
        <input name="year" value="<?php echo e(request('year')); ?>" placeholder="Year" type="number" class="bg-white/10 border border-white/20 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
        <input name="min_price" value="<?php echo e(request('min_price')); ?>" placeholder="Min Price" type="number" step="0.01" class="bg-white/10 border border-white/20 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
        <input name="max_price" value="<?php echo e(request('max_price')); ?>" placeholder="Max Price" type="number" step="0.01" class="bg-white/10 border border-white/20 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
        <select name="status" class="bg-white/10 border border-white/20 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
            ...
            <?php $__currentLoopData = ['available', 'reserved', 'sold']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($status); ?>" <?php if(request('status') === $status): echo 'selected'; endif; ?> class="bg-gray-800"><?php echo e(ucfirst($status)); ?></option>
...
<div class="flex justify-between items-center mb-6">
    <div>
        <p class="text-sm text-gray-400">Total: <span class="font-semibold text-white"><?php echo e($vehicles->total()); ?></span> vehicles</p>
    </div>
    <a href="<?php echo e(route('vehicles.create')); ?>" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg transition-all flex items-center gap-2 font-medium shadow-lg">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    <?php $__empty_1 = true; $__currentLoopData = $vehicles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vehicle): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="glass-card rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300">
            ...
            <div class="relative h-48 bg-gradient-to-br from-gray-800 to-gray-900 overflow-hidden">
                ...
                <div class="absolute top-3 right-3">
                    <span class="inline-flex px-3 py-1 rounded-full text-xs font-semibold shadow-lg backdrop-blur-sm
                        <?php echo e($vehicle->status === 'available' ? 'bg-emerald-500/90 text-white' : ''); ?>

                        <?php echo e($vehicle->status === 'reserved' ? 'bg-blue-500/90 text-white' : ''); ?>

                        <?php echo e($vehicle->status === 'sold' ? 'bg-slate-500/90 text-white' : ''); ?>">
            ...
            <div class="p-5">
                <!-- Vehicle Name -->
                <h3 class="text-xl font-bold text-white mb-2"><?php echo e($vehicle->brand); ?> <?php echo e($vehicle->model); ?></h3>
                
                <!-- Vehicle Details -->
                <div class="space-y-2 mb-4">
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-gray-400">Year</span>
                        <span class="font-semibold text-gray-200"><?php echo e($vehicle->year); ?></span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-gray-400">Mileage</span>
                        <span class="font-semibold text-gray-200"><?php echo e(number_format($vehicle->mileage)); ?> km</span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-gray-400">Seller</span>
                        <span class="font-semibold text-gray-200"><?php echo e($vehicle->seller->name ?? '-'); ?></span>
                    </div>
                </div>

                <!-- Description Preview -->
                <?php if($vehicle->description): ?>
                    <p class="text-sm text-gray-300 mb-4 line-clamp-2"><?php echo e($vehicle->description); ?></p>
                <?php endif; ?>

                <!-- Price -->
                <div class="border-t border-white/10 pt-4 mb-4">
                    <div class="flex items-baseline gap-1">
                        <span class="text-2xl font-bold text-green-400">Nu. <?php echo e(number_format($vehicle->price, 2)); ?></span>
                    </div>
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <form method="POST" action="<?php echo e(route('vehicles.destroy', $vehicle)); ?>" onsubmit="return confirm('Are you sure you want to delete this vehicle?')" class="inline">
...
<div class="mt-6"><?php echo e($vehicles->links()); ?></div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /opt/lampp/htdocs/CBS/resources/views/vehicles/index.blade.php ENDPATH**/ ?>